...
//******************PWM引脚和电机驱动引脚***************************//
const int AIN1 = 5;//A电机PWM波 定时器3
const int AIN2 = 6;//A电机PWM波 定时器4
const int BIN1 = 8;//B电机PWM波 定时器4
const int BIN2 = 7;//B电机PWM波 定时器4
const int CIN1 = 11;//C电机PWM波 定时器1
const int CIN2 = 12;//C电机PWM波 定时器1
const int DIN1 = 44;//D电机PWM波 定时器5
const int DIN2 = 46;//D电机PWM波 定时器5
//*********************************************//
int Transformation;   //APP摇杆-按键控制模式切换标识
int Plug=0;//手柄数据相关标志
float Velocity_KP =1.05, Velocity_KI =0.015;//PI参数
int Flag_Way=0,Flag_Direction;             //遥控模式标志与蓝牙串口通信相关标志Flag_Way--> 0：PS2 1:AUTO
...
void Set_PWM(int motora, int motorb, int motorc,int motord) { 
  if (motora > 0)        analogWrite(AIN2, motora+100), analogWrite(AIN1, 0); //赋值给PWM寄存器根据电机响应速度与机械误差微调,
  else if(motora == 0)   analogWrite(AIN2, 0), analogWrite(AIN1, 0);   
  else if (motora < 0)   analogWrite(AIN1, -motora+100), analogWrite(AIN2, 0);//高频时电机启动初始值高约为130，低频时电机启动初始值低约为30

  if (motorb > 0)        analogWrite(BIN2,motorb+100),analogWrite(BIN1, 0); //赋值给PWM寄存器根据电机响应速度与机械误差微调,
  else if(motorb == 0)   analogWrite(BIN2, 0),analogWrite(BIN1, 0);
  else if (motorb < 0)   analogWrite(BIN1,-motorb+100),analogWrite(BIN2, 0);//高频时电机启动初始值高约为130，低频时电机启动初始值低约为30

  if (motorc > 0)        analogWrite(CIN1,motorc+100),analogWrite(CIN2,0); //赋值给PWM寄存器根据电机响应速度与机械误差微调,
  else if(motorc == 0)   analogWrite(CIN2, 0), analogWrite(CIN1, 0);
  else if (motorc < 0)   analogWrite(CIN2, -motorc+100), analogWrite(CIN1, 0);//高频时电机启动初始值高约为130，低频时电机启动初始值低约为30

  if (motord > 0)        analogWrite(DIN1,motord+100),analogWrite(DIN2, 0); //赋值给PWM寄存器根据电机响应速度与机械误差微调,
  else if(motord == 0)   analogWrite(DIN1, 0), analogWrite(DIN2, 0);
  else if (motord < 0)   analogWrite(DIN2, -motord+100), analogWrite(DIN1, 0);//高频时电机启动初始值高约为130，低频时电机启动初始值低约为30
}
/**************************************************************************
函数功能：增量PI控制器
入口参数：编码器测量值，目标速度
...
{  float Bias; 
   ...
   Last_bias=Bias;                                       //保存上一次偏差
   if(Flag_Way == 1){if (Flag_Direction == 0){if(Target>0)PWM = -2,Last_bias = 0;if(Target<0)PWM = 2,Last_bias = 0;}}//APP模式停止时，重置相关的变量，反向赋值降低小车惯性移动距离
   else if(Flag_Way == 0){if (abs(PS2_Velocity)<=51){if(Target>0)PWM = -2,Last_bias = 0;if(Target<0)PWM = 2,Last_bias = 0;}}//手柄模式停止时，重置相关的变量，反向赋值降低小车惯性移动距离
   return PWM;                                           //增量输出
}
/**************************************************************************/
...
{     if (Flag_Direction == 0)PWMA=-Gryo*Bluetooth_Velocity/3300,PWMB=-Gryo*Bluetooth_Velocity/3300,PWMC=+Gryo*Bluetooth_Velocity/3300,PWMD=+Gryo*Bluetooth_Velocity/3300;
 else if (Flag_Direction == 1)PWMA=-Gryo*Bluetooth_Velocity/3300,PWMB=-Gryo*Bluetooth_Velocity/3300,PWMC=+Gryo*Bluetooth_Velocity/3300,PWMD=+Gryo*Bluetooth_Velocity/3300;  
 if (Transformation == 0){
      if (Flag_Direction == 5)PWMA=-Gryo*Bluetooth_Velocity/3300,PWMB=-Gryo*Bluetooth_Velocity/3300,PWMC=+Gryo*Bluetooth_Velocity/3300,PWMD=+Gryo*Bluetooth_Velocity/3300;
 else if (Flag_Direction == 7)PWMA=-Gryo*Bluetooth_Velocity/3300,PWMB=-Gryo*Bluetooth_Velocity/3300,PWMC=+Gryo*Bluetooth_Velocity/3300,PWMD=+Gryo*Bluetooth_Velocity/3300;
 else if (Flag_Direction == 3)PWMA=-Gryo*Bluetooth_Velocity/3300,PWMB=-Gryo*Bluetooth_Velocity/3300,PWMC=+Gryo*Bluetooth_Velocity/3300,PWMD=+Gryo*Bluetooth_Velocity/3300;
 else if (Flag_Direction == 8)PWMA=-Gryo*Bluetooth_Velocity/3000,PWMB=0,PWMC=+Gryo*Bluetooth_Velocity/3000,PWMD=0;
 else if (Flag_Direction == 4)PWMA=-Gryo*Bluetooth_Velocity/3000,PWMB=0,PWMC=+Gryo*Bluetooth_Velocity/3000,PWMD=0;
 else if (Flag_Direction == 2)PWMA=0,PWMB=-Gryo*Bluetooth_Velocity/3000,PWMC=0,PWMD=+Gryo*Bluetooth_Velocity/3000;
 else if (Flag_Direction == 6)PWMA=0,PWMB=-Gryo*Bluetooth_Velocity/3000,PWMC=0,PWMD=+Gryo*Bluetooth_Velocity/3000;
  }}
/**************************************************************************
函数功能：异常关闭电机
入口参数：电压
...
void control() {
  ...
  sei();//全局中断开启
  Velocity_A = -Velocity_1;    Velocity_1 = 0; //读取编码器数据并根据实际接线做调整、然后清零，这就是通过M法测速（单位时间内的脉冲数）得到速度。除1.5？清零周期
  ...
  PS2_Velocity =3.5*(abs(LY)+abs(LX));//取摇杆数据为目标速度与摇杆力度和角度相关
  Get_RC();//遥控函数
  if(Flag_Way == 1){//蓝牙遥控赋值
  Multiple=2*Bluetooth_Velocity/80;//目标速度大，加快启动加速过程
  if(Bluetooth_Velocity<=480)Multiple=8.5; //目标速度小，延长启动加速过程
  ...
  if(abs(PS2_Velocity)<=480)Multiple=8.5;  //目标速度小，延长启动加速过程 
  ...
  if (Turn_Off() == 0) Set_PWM(Motora, Motorb,Motorc,Motord ); //如果不存在异常，使能电机  
...
void event() 
{
  ...
  while (Serial3.available())
  {
    ...
    if (ReceiveInstruction == 'J')Transformation = 0;//字符J对应APP摇杆遥控模式标志
    if (ReceiveInstruction == 'K')Transformation = 1;//字符K对应APP按键遥控模式标志
    if (ReceiveInstruction >= 0x41 && ReceiveInstruction <= 0x48)        Flag_Direction = ReceiveInstruction - 0x40;//接收APP指令，ASCLL码对应字符A~H
    else  if (ReceiveInstruction < 10)        Flag_Direction = ReceiveInstruction;//字符A~H对应转换为1到8
    else  if (ReceiveInstruction == 0X5A)        Flag_Direction = 0;//字符Z对应转换为0
    else  Flag_Direction = 9; //非操控指令统一返回参数9，防止蓝牙连接断开导致小车不受控制
    if (ReceiveInstruction == 0x7B) Flag_PID = 1; //参数指令起始位
    if (ReceiveInstruction == 0x7D) Flag_PID = 2; //参数指令停止位
    if (Flag_PID == 1)      Receive[i] = ReceiveInstruction,        i++;
...
//     if (Flag_Direction == 0){VelocityA =0;VelocityB =0;VelocityC =0;VelocityD =0;}//停止，速度为0
...
//    if (Flag_Direction == 0) {VelocityA =0;VelocityB =0;VelocityC =0;VelocityD =0;}
...
//    Velocity__D = -(3.5*(-LX+LY+RX*1)-Gryo*(abs(LX)+abs(LY))/80);//手柄运动控制函数，3.5为设定摇杆速度倍率，可根据需要调整 
        Velocity__A = 3.5*(LX-LY);//手柄运动控制函数，3.5为设定摇杆速度倍率，可根据需要调整
        Velocity__D = -3.5*(LX+LY);//手柄运动控制函数，3.5为设定摇杆速度倍率，可根据需要调整 
    Velocity__B = Velocity__A;
    Velocity__C = Velocity__D;
    ...
    Velocity__B = 0;
    Velocity__C = 0;
    Velocity__D = 0;  }//非断电状态直接拔手柄无线模块速度置零，防止小车不受控制       
}  
}
/********函数功能：OLED显示*********/
void OLED()
{
  ...
  if(Flag_Way == 1){oled.drawstring(00, 3, "Target-Velocity:");//显示APP设定的目标速度
   OLED_ShowNumber(100, 3,Bluetooth_Velocity , 4);}            //显示APP设定的目标速度
  if(Flag_Way == 0){oled.drawstring(00, 3, "Target-Velocity:");//显示手柄设定的目标速度
   OLED_ShowNumber(100, 3,abs(PS2_Velocity) , 4);}             //显示手柄设定的目标速度
    oled.drawstring(00, 04, "Mode-select:");
   if(Flag_Way == 1){oled.drawstring(95, 04, "AUTO");}
   if(Flag_Way == 0){oled.drawstring(95, 04, "PS2");}
...
void setup() {
  ...
  TCCR1B =(TCCR1B & 0xF8) | fff;//调整计数器分频，频率调高至31.374KHZ pwm用
...
void READ_ENCODER_A() {
  if (digitalRead(DIRECTION_A) == LOW) {     //如果是下降沿触发的中断
    if (digitalRead(ENCODER_A) == LOW)      Velocity_1--;  //根据另外一相电平判定方向
    else      Velocity_1++;
  }
  else {     //如果是上升沿触发的中断
    if (digitalRead(ENCODER_A) == LOW)      Velocity_1++; //根据另外一相电平判定方向
...
void READ_ENCODER_B() {
  if (digitalRead(DIRECTION_B) == LOW) { //如果是下降沿触发的中断
    if (digitalRead(ENCODER_B) == LOW)      Velocity_2++;//根据另外一相电平判定方向
    else      Velocity_2--;
  }
  else {   //如果是上升沿触发的中断
    if (digitalRead(ENCODER_B) == LOW)      Velocity_2--; //根据另外一相电平判定方向
...
void READ_ENCODER_C() {
  if (digitalRead(DIRECTION_C) == LOW) { //如果是下降沿触发的中断
    if (digitalRead(ENCODER_C) == LOW)      Velocity_3++;//根据另外一相电平判定方向
    else      Velocity_3--;
  }
  else {   //如果是上升沿触发的中断
    if (digitalRead(ENCODER_C) == LOW)      Velocity_3--; //根据另外一相电平判定方向
...
void READ_ENCODER_D() {
  if (digitalRead(DIRECTION_D) == LOW) { //如果是下降沿触发的中断
    if (digitalRead(ENCODER_D) == LOW)      Velocity_4++;//根据另外一相电平判定方向
    else      Velocity_4--;
  }
  else {   //如果是上升沿触发的中断
    if (digitalRead(ENCODER_D) == LOW)      Velocity_4--; //根据另外一相电平判定方向
...
void loop() {  
 ...
 if(Plug==0)ps2x.read_gamepad(false, 0); //read controller and set large motor to spin at 'vibrate' speed
    //if(Plug==0)ps2x.read_gamepad(); //read controller and set large motor to spin at 'vibrate' speed
  ...
  if (LX > -Yuzhi && LX < Yuzhi)LX = 0; //小角度设为死区 防止抖动出现异常
  if (RX > -Yuzhi && RX < Yuzhi)RX = 0; //小角度设为死区 防止抖动出现异常 
  if(PS2_RY>133){Flag_Way = 0;}//手柄右摇杆下拨，进入手柄模式
//  if(PS2_RY<120){Flag_Way = 1;}//手柄右摇杆上拨，进入AUTO模式PS2
//  if(PS2_RY==255&&PS2_RX==255&&PS2_LY==255&&PS2_LX==255){Flag_Way = 1;Plug=1;}//拔掉手柄接收模块，进入AUTO模式
...
unsigned char  select(void){
  ...
  PS2_LX=ps2x.Analog(PSS_LX);//手柄左摇杆X轴数据读取
  PS2_RY=ps2x.Analog(PSS_RY);//手柄右摇杆Y轴数据读取
  if(PS2_RY==255&&PS2_RX==255&&PS2_LY==255&&PS2_LX==255){Flag_Way = 1;Plug=1;flag=0;}//不插手柄接收模块，禁止进入手柄模式，直接进入AUTO模式
