...
void control() {
  ...
  sei();//全局中断开启
   //获取电机A速度
  if (Velocity_1>-EncoderConstrain && Velocity_1<EncoderConstrain)
  {
...
//    }
Velocity_B =  Velocity_2;    Velocity_2 = 0;
  //获取电机C速度
  if (Velocity_3>-EncoderConstrain && Velocity_3<EncoderConstrain)
  {
 Velocity_C = -Velocity_3;    Velocity_3 = 0; //读取编码器数据并根据实际接线做调整、然后清零，这就是通过M法测速（单位时间内的脉冲数）得到速度。
  ...
  else
  {
   ...
   //获取电机D速度
  if (Velocity_4>-EncoderConstrain && Velocity_4<EncoderConstrain)
  {
